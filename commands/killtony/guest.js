    ...
    async execute(interaction) {
        ...
        for (const episodeNumber in data) {
            ...
            for (const guest of guests) {
                if (guest.toLowerCase().includes(searchName)) {
                    episodesFound.push(`[#${episodeNumber}](${episode.url})  –  ${guest}`);
        ...
        const filter = (i) => i.isButton() && ['first', 'previous', 'next', 'last'].includes(i.customId);
